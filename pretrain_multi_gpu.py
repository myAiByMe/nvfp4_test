...
def train_epoch(
    ...
    # Sampler : DistributedSampler en multi-GPU, séquentiel en single-GPU
    if WORLD_SIZE > 1:
        ...
        if IS_MASTER:
            print(f"  DistributedSampler : n={total_seqs:,}  GPUs={WORLD_SIZE}  "
                  f"per_gpu={total_seqs // WORLD_SIZE:,}")
    else:
    ...
    for batch_idx, batch in enumerate(pbar):
        try:
            ...
            if (accumulated_steps % CONFIG['gradient_accumulation'] == 0) or is_last:
                ...
                if global_step % CONFIG['validate_every_steps'] == 0 and IS_MASTER:
                ...
                if global_step % CONFIG['save_every_steps'] == 0 and IS_MASTER:
...
def main():
    ...
    if CONFIG['use_compile'] and device != 'cpu':
        if IS_MASTER: print('torch.compile...')
        import torch._dynamo
        torch._dynamo.config.cache_size_limit = 256
        torch._dynamo.config.suppress_errors  = True
        try:
            model = torch.compile(model, mode=CONFIG['compile_mode'])
            if IS_MASTER: print('  OK')
        except Exception as e:
            if IS_MASTER: print(f'  FAIL : {e}')

    # ── Wrap DDP après compile ───────────────────────────────────
    if WORLD_SIZE > 1:
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK,
        )
        if IS_MASTER: print(f'  DDP activé : {WORLD_SIZE} GPUs')
    ...
    # Unwrap DDP puis compile pour accéder aux vrais paramètres
    raw_model = model
    if hasattr(raw_model, 'module'):       # DDP
...
if __name__ == '__main__':
    try:
        ...
        if IS_MASTER: print(traceback.format_exc())
    finally:
        if IS_MASTER: print('\nBye')
